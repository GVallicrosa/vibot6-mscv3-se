...
void ScreenShot(std::string Fichier)
{
     	// Pour gérer le buffer
...
//	char Fichier[50] = Fichiersd.c_str();
	...
	// On incrémente le nombre de screenshot
	Nombre_Screenshot ++;

	// Alloaction de la mémoire pour lire depuis le frame buffer
	if (!(buffer = (unsigned char *) calloc(1, buf_size)))
		return;


	// Informations de l'en-tête du fichier
	buffer[2] = 2;			// Non-compressé
	buffer[12] = w & 255;
	buffer[13] = w >> 8;
	buffer[14] = h & 255;
	buffer[15] = h >> 8;
	...
	// Ecriture de l'en-tête du fichier + l'image dans le fichier
...
Vector3d DoubleToGreyDiscrete( const double d, double n ){
...
void RFunctions()
{
    ...
    for( int i=0; i<n+1; i++) Coord[i] = new double[n+1];
    ...
    for( int i = 0; i<n+1; i++)
        for( int j= 0 ; j< n+1; j++)
        {
            ...
            Vector2d Df1 = Vector2d(
                -(P[0] - C1[0]) / (P-C1).norm(),
                -(P[1] - C1[1]) / (P-C1).norm() );

            f4 = r2 - (P-C2).norm();

            Vector2d Df4 = Vector2d(
                -(P[0] - C2[0]) / (P-C2).norm(),
                -(P[1] - C2[1]) / (P-C2).norm() );

            f3 = r3 - (P-C3).norm();

            Vector2d Df3 =  Vector2d(
                -(P[0] - C3[0]) / (P-C3).norm(),
                -(P[1] - C3[1]) / (P-C3).norm());

            f2 = r4 - (P-C4).norm();

            Vector2d Df2 =  Vector2d (
                -(P[0] - C4[0]) / (P-C4).norm(),
                -(P[1] - C4[1]) / (P-C4).norm());
            ...
            Vector2d Dtmp1 = Vector2d(
                    Df1[0] + Df2[0] + (Df1[0]*f1 + Df2[0]*f2)/sqrt(f1*f1+f2*f2),
                    Df1[1] + Df2[1] + (Df1[1]*f1 + Df2[1]*f2)/sqrt(f1*f1+f2*f2));
            ...
            Vector2d Df = Vector2d (
                Dtmp1[0] + Dtmp2[0] + (Dtmp1[0]*tmp1 + Dtmp2[0]*tmp2)/sqrt(tmp1*tmp1+tmp2*tmp2),
                Dtmp1[1] + Dtmp2[1] + (Dtmp1[1]*tmp1 + Dtmp2[1]*tmp2)/sqrt(tmp1*tmp1+tmp2*tmp2)
...
cout <<"FMIN="<<fmin<<" FMAX="<<fmax<<endl;
    ...
    for( int i = 0; i<n; i++)
        for( int j= 0 ; j<n; j++)
        {
            ...
            Vector3d col1 = DoubleToColorDiscrete((Coord[i][j]-fmin)/(fmax-fmin));
            ...
            Vector3d col2 = DoubleToColorDiscrete((Coord[i+1][j]-fmin)/(fmax-fmin));
            ...
            Vector3d col3 = DoubleToColorDiscrete((Coord[i+1][j+1]-fmin)/(fmax-fmin));

            glColor3f(col3[0],col3[1],col3[2]);
            glVertex3f( -5 + (i+1)*gap, -5+(j+1)*gap, Coord[i+1][j+1]);
            //glVertex3f( -5 + (i+1)*gap, -5+(j+1)*gap, 0);

            Vector3d col4 = DoubleToColorDiscrete((Coord[i][j+1]-fmin)/(fmax-fmin));
    ...
    for (int i=0; i<100; i++)
    {
        P = Vector3d(C1[0],C1[1],0) + Vector3d(r1*cos(2*i*PI/100), r1*sin(2*i*PI/100),0);
    ...
    for (int i=0; i<100; i++)
    {
        P = Vector3d(C2[0],C2[1],0) + Vector3d(r2*cos(2*i*PI/100), r2*sin(2*i*PI/100),0);
        glVertex3f(P[0],P[1],P[2]);
    }
    glEnd();
    ...
    for (int i=0; i<100; i++)
    {
        P = Vector3d(C3[0],C3[1],0) + Vector3d(r3*cos(2*i*PI/100), r3*sin(2*i*PI/100),0);
        glVertex3f(P[0],P[1],P[2]);
    }
    glEnd();
    ...
    for (int i=0; i<100; i++)
    {
        P = Vector3d(C4[0],C4[1],0) + Vector3d(r4*cos(2*i*PI/100), r4*sin(2*i*PI/100),0);
        glVertex3f(P[0],P[1],P[2]);
    }
    glEnd();
...
cout <<RS.radius(0) <<" "<<RS.radius(0)<<endl;
    ...
    for( int i = 0; i<n; i++)
        for( int j= 0 ; j<n; j++)
        {
            ...
            if ( FlatOn) glVertex3f( Pmin[0] + i*gap, Pmin[1]+j*gap, 0);
            else glVertex3f( Pmin[0] + i*gap, Pmin[1]+j*gap, Coord[i][j]);


            glColor3f(col2[0],col2[1],col2[2]);
            glColor3f(AVG[0],AVG[1],AVG[2]);
            if ( FlatOn ) glVertex3f( Pmin[0] + (i+1)*gap, Pmin[1]+j*gap, 0);
            else glVertex3f( Pmin[0] + (i+1)*gap, Pmin[1]+j*gap, Coord[i+1][j]);



            glColor3f(col3[0],col3[1],col3[2]);
            glColor3f(AVG[0],AVG[1],AVG[2]);
            if ( FlatOn ) glVertex3f( Pmin[0] + (i+1)*gap, Pmin[1]+(j+1)*gap, 0);
            else glVertex3f( Pmin[0] + (i+1)*gap, Pmin[1]+(j+1)*gap, Coord[i+1][j+1]);


            glColor3f(col4[0],col4[1],col4[2]);
            glColor3f(AVG[0],AVG[1],AVG[2]);
            if (FlatOn) glVertex3f( Pmin[0] + (i)*gap, Pmin[1]+(j+1)*gap,0);
            else glVertex3f( Pmin[0] + (i)*gap, Pmin[1]+(j+1)*gap, Coord[i][j+1]);
...
void gaussj2(double **a, int n, double **b, int m)
...

{
	...
	for (i=0;i<n;i++) { //This is the main loop over the columns to be	reduced.
		...
		if (irow != icol) {
			for (l=0;l<n;l++) {tmp=a[irow][l];a[irow][l]=a[icol][l]; a[icol][l]=tmp;}
			for (l=0;l<m;l++) {tmp=b[irow][l];b[irow][l]=b[icol][l]; b[icol][l]=tmp;}
		}
	...
	for (l=n-1;l>=0;l--) {
		if (indxr[l] != indxc[l])
			for (k=0;k<n;k++){tmp=a[k][indxr[l]]; a[k][indxr[l]]=a[k][indxc[l]]; a[k][indxc[l]]= tmp;}
	} //And we are done.
...
bool gaussj_new(double **a, int n, double**b){
...
bool gaussj_new_updated(double **a, int n, double**b){
...
*/
    ...
    for( int i = 0; i<n; i++)
        for( int j= 0 ; j<n; j++)
        {
            ...
            Vector3d col1;
            if (Coord[i][j] <0) col1 = (Coord[i][j]/fmin)*Vector3d(1,1,1);
            else col1= DoubleToColorDiscrete(Coord[i][j]/fmax);
            glColor3f(col1[0],col1[1],col1[2]);
            glVertex3f( -5 + i*gap, -5+j*gap, 0);


            Vector3d col2;
            if (Coord[i+1][j] <0) col2 = (Coord[i+1][j]/fmin)*Vector3d(1,1,1);
            else col2= DoubleToColorDiscrete(Coord[i+1][j]/fmax);
            glColor3f(col2[0],col2[1],col2[2]);
            glVertex3f( -5 + (i+1)*gap, -5+j*gap, 0);

            Vector3d col3;
            if (Coord[i+1][j+1] <0) col3 = (Coord[i+1][j+1]/fmin)*Vector3d(1,1,1);
            else col3= DoubleToColorDiscrete(Coord[i+1][j+1]/fmax);
            glColor3f(col3[0],col3[1],col3[2]);
            glVertex3f( -5 + (i+1)*gap, -5+(j+1)*gap, 0);

            Vector3d col4;
            if (Coord[i][j+1] <0) col4 = (Coord[i][j+1]/fmin)*Vector3d(1,1,1);
            else col4= DoubleToColorDiscrete(Coord[i][j+1]/fmax);
            glColor3f(col4[0],col4[1],col4[2]);
            glVertex3f( -5 + (i)*gap, -5+(j+1)*gap,0);
...
void OBB(vector < Vector2d > Data, Vector2d &T, Vector2d &R, Vector2d &S)
{
    T << 0,0;
    R << 0,0;
    S << 1e15,1e15;
...
Vector2d dum(1e15,1e15);
  ...
  /*  double n = test.row(0).norm();
    ...
    if( fabs(test.determinant() - 1.) > 1e-6) {cout <<" Determinant should be 1 after normalization!!"<<endl; system("pause");}
...
vector < Vector2d > GenerateOutlier( vector < Vector2d > Data, double outlier_percentage, double radiusratio )
{
    vector <Vector2d> NewData;

    //compute average
    Vector2d C(0,0);
    for (int i=0; i<Data.size(); i++) {C+=Data[i];NewData.push_back(Data[i]);}
    C /= Data.size();

    //compute max distance from center
    double maxdist = 0;
    for (int i=0; i<Data.size(); i++) maxdist = max(maxdist, (C-Data[i]).norm());
...
cout <<"OK"<<endl;
    ...
    while( getline( datafile, line ) )
	{
		...
		if(node.size() != 2) {cout<<"Syntax error while readind data : only two coordinates per line are supported"<<endl;exit(0);}
...
vector <Vector2d> RemoveData (vector < Vector2d > Data, Vector3d Plane)
{
