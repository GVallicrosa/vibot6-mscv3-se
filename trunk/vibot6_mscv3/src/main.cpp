...
double a_scale_1, a_offset_1, b_scale_1, b_offset_1, n_scale_1, n_offset_1,
    tht_offset_1, tht_scale_1, phi_offset_1, phi_scale_1, x_offset_1,
    x_scale_1, y_offset_1, y_scale_1, z_offset_1, z_scale_1;

double RADIALNOISEPERCENTAGE, OPTIMALERRORSTOP, PROBA_MUTATE, RANDOMRADIUS,
    kmutate, thresholdvariance;

double a_scale_2, a_offset_2, b_scale_2, b_offset_2, n_scale_2, n_offset_2,
    tht_offset_2, tht_scale_2, phi_offset_2, phi_scale_2, x_offset_2,
    x_scale_2, y_offset_2, y_scale_2, z_offset_2, z_scale_2;
...
main(int argc, char ** argv)
{
  ...
  Init();
  Vector2d ImRes;
  LoadJPG("Images/Different0035.jpg", 0, ImRes);
  ...
  while (getline(infile, line))
    {
      ...
      if (node.size() != 2)
        {
          cout << "Syntax error while readind data" << endl;
          exit(0);
        }
  ...
  for (int i = 0; i < 4; i++)
    {
      ...
      tmp.Optimize2(SmoothData, ErrorOfFit, false, // activate normalization or not
  ...
  glBegin(GL_QUADS);
  glTexCoord2f(0, 0);
  glVertex3d(-ImRes[0] * 0.5, ImRes[1] * 0.5, 0);
  glTexCoord2f(0, 1);
  glVertex3d(-ImRes[0] * 0.5, -ImRes[1] * 0.5, 0);
  glTexCoord2f(1, 1);
  glVertex3d(ImRes[0] * 0.5, -ImRes[1] * 0.5, 0);
