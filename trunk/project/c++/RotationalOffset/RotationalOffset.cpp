...
void cRotationalOffset::CartisanToPolar(vector< pair<unsigned int, unsigned int> > ContourPtr)
{
	//unsigned int i;
	...
	//Normalise the COM
	vXc /= ContourPtr.size();//mNoOfPoints;
	vYc /= ContourPtr.size();//mNoOfPoints;
	...
	for (i = ContourPtr.begin(); i != ContourPtr.end(); i++) {
		vX		 = i->first - vXc;
		vY		 = i->second - vYc;
		vRad = static_cast<float> (sqrt(static_cast<double>(vX*vX) + static_cast<double>(vY*vY)));

		vTheta = static_cast<float> (atan2(static_cast<double> (vY), static_cast<double> (vX))); /*Gives the output in range -PI to PI */
...
void cRotationalOffset::FindMinimum(void)
{
	float vCurr			= 0;
	float vPrev			= 0;
	float vPrev2		= 0;float vPrev8			= 0;float vPrev9			= 0;float vPrev10			= 0;
	float vPrev3		= 0;float vPrev4			= 0;float vPrev5			= 0;float vPrev6			= 0;float vPrev7			= 0;
	float vNext			= 0;float vNext3			= 0;float vNext5			= 0;float vNext7			= 0;float vNext9			= 0;
	float vNext2		= 0;float vNext4			= 0;float vNext6			= 0;float vNext8			= 0;float vNext10			= 0;
	...
	for(i = mThetaRad.begin() + 10; i != mThetaRad.end() - 10; i++) {
		...
		if(vCurr < vPrev && vCurr < vPrev2 && vCurr < vNext && vCurr < vNext2 &&
          ...
          vCurr < vPrev9 && vCurr < vPrev10 && vCurr < vNext9 && vCurr < vNext10) {
			...
			mpOutput[count].push_back(i->first);
			//count++;
		}
	}
...
vector< vector<float> >	 cRotationalOffset::GetMinRadius(vector< vector< pair<unsigned int, unsigned int> > > ContoursPtr)
{
	vector< vector<pair<unsigned int, unsigned int> > >::iterator i;

	for (i = ContoursPtr.begin(); i != ContoursPtr.end(); i++) {
		CartisanToPolar(*i);
		/*Sort in pairs ascending order.
		By default sort function sort on the basis of 1st element of pairs. Thats exactly we want !! Sort radius on the basis of theta.
