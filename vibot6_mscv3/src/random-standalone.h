...
public:
  ...
  template<typename T>
    ...
    uniform(T min, T max)
    {
      return static_cast<T> (min + (max + (typeIsInteger<T> () ? 1 : 0) - min)
          * theRandom());
    }
  ...
  double
  gaussian(double mean = 0, double standardDeviation = 1);
...
private:
  //methodes "interdites" / "forbidden" methods
  Random(const Random&)
  {
  }
  ;
