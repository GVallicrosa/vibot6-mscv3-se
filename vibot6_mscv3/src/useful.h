...
Vector3d
DoubleToColor(double d);
Vector3d
DoubleToColorDiscrete(const double d, double n = 20.0);
Vector3d
DoubleToGreyDiscrete(const double d, double n = 20.0);
...
//OpenGL text display
void
PrintMessage(int, int, const std::string);
...
//Export screenshot
void
ScreenShot(std::string Fichiersdfsd);
void
...
void
OBB(vector<Vector2d> , Vector2d &T, Vector2d &R, Vector2d &S);

vector<Vector2d>
GenerateOutlier(vector<Vector2d> , double outlier_percentage = 0.1,
    double radiusratio = 2);
vector<Vector2d>
RemoveData(vector<Vector2d> , Vector3d Plane);
vector<Vector2d>
