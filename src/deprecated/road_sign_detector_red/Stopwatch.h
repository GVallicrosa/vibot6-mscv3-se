...
class Stopwatch
{
	...
	public :
		...
		inline void PrintInter(std::ostream& os = std::cout) const {
			os<<inter/CLOCKS_PER_SEC<<'.'<<std::setw(2)<<std::setfill('0')<<(int)((float)inter/CLOCKS_PER_SEC*100)%100<<std::setfill(' ')<<"s";
		}

		// Print total elapsed time
		inline void PrintTotal(std::ostream& os = std::cout) const {
			os<<total/CLOCKS_PER_SEC<<'.'<<std::setw(2)<<std::setfill('0')<<(int)((float)total/CLOCKS_PER_SEC*100)%100<<std::setfill(' ')<<"s";
		}
